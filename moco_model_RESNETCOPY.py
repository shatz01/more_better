...
class MocoModel(pl.LightningModule):
    def __init__(self, memory_bank_size, moco_max_epochs, downstream_max_epochs=0, dataloader_train_classifier=None, dataloader_test=None, downstream_test_every=0, last_epoch=-1):
        ...
        # .load_from_checkpoint("./saved_models/resnet_80/epoch=73-val_loss=0.43-val_acc=0.90.ckpt")
        resnet = plr18().load_from_checkpoint("./saved_models/resnet_80/epoch=73-val_loss=0.43-val_acc=0.90.ckpt").model
        for p in resnet.parameters():
            p.requires_grad = False
        # self.resnet = plr18.plr18().load_from_checkpoint("./saved_models/resnet_80/epoch=79-val_loss=0.70-val_acc=0.80.ckpt")
        # self.resnet.freeze()
    ...
    def configure_optimizers(self):
        optim_params = [p for p in self.resnet_moco.parameters() if p.requires_grad]
        optim = torch.optim.SGD(optim_params, lr=6e-2,
                                momentum=0.9, weight_decay=5e-4)
        # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optim, self.moco_max_epochs, last_epoch=self.last_epoch) 
        scheduler = {
            "scheduler": torch.optim.lr_scheduler.ReduceLROnPlateau(optim, factor=0.9, patience=10),
